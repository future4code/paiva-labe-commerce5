...
export default class App extends React.Component {
  ...
  onAdicionarCarrinho = (idProduto) => {
    const produtoCarrinho = this.state.produtosNoCarrinho.find(produto => idProduto === produto.id)

    if (produtoCarrinho) {
      const novaListaNoCarrinho = this.state.produtosNoCarrinho.map((produto) => {
      ...
      this.setState({ produtosNoCarrinho: novaListaNoCarrinho })
    } else {
      const produtosParaAdd = products.find(produto => idProduto === produto.id)

      const novoListaDeProduto = [...this.state.produtosNoCarrinho, { ...produtosParaAdd, quantidade: 1 }]
      console.log(novoListaDeProduto)
      this.setState({ produtosNoCarrinho: novoListaDeProduto })
    }
  }

  onRemoverCarrinho = (idProduto) => {
    const produtoCarrinho = this.state.produtosNoCarrinho.find(produto => idProduto === produto.id)
  ...
  render() {
    return (
      <AppContainer>
        ...
        <Produto
          products={products}
          onAdicionarCarrinho={this.onAdicionarCarrinho}
        />
        <Carrinho
          produtosNoCarrinho={this.state.produtosNoCarrinho}
          onRemoverCarrinho= {this.onRemoverCarrinho}
          />
